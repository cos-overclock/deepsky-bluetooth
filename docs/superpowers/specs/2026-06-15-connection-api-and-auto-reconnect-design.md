...
| 24 | 再接続失敗の通知 | 初回一時失敗/リンク消失と終端失敗は `connectionStates` の `disconnected(reason: ...)` で観測可能。`bluetoothOff` は維持要求を残して電源復帰待ち、非搭載は `bluetoothUnavailable` で終端。`reconnecting` 中の反復一時失敗はObserverだけへ記録する |
| 25 | services キャッシュ | epoch退役時に `device.services` を即座に `null` へ戻す。旧handleを含むキャッシュを次epochへ持ち越さない |
| 26 | handoverバッファ | 最大256イベントまたは30秒。超過時は古いnotify/presenceから破棄し、状態snapshotを正としてObserverへ警告 |
...
`createTransport(config, observer)` はplatform解決と `initialize(config)` までを行い、
...
bridge/platformのObserverは生のtransport呼び出しを、lifecycleのObserverは公開API呼び出しを観測する。
...
- 現段階では `DeepskyBluetooth.foreground()` / `background()` が
  `createTransport(config, observer)` で初期化済みの `BleTransport` を1つ生成し、
...
- `autoReconnect == true` の一時的な想定外切断 → `disconnected(reason)` 発行後 `reconnecting` へ。以降は選択中の駆動源が次の試行を起こす:
  - A: `delay` 待機 → platform connect → 一時失敗なら公開状態は `reconnecting` のまま維持し再度
    `delay`。各試行失敗はObserverへ記録するが、`disconnected`/`reconnecting` を反復発行しない。
...
`reconnecting` 中の一時失敗はObserverだけへ記録する。終端理由を受けた場合は現在状態にかかわらず
...
- バッファリング: active sink が無い期間およびhandover中のイベントは owner が短期バッファし、
  ...
  接続状態は最新snapshotへ集約して保持する。破棄件数と理由はObserverへ警告し、Dart readyが来ないため
