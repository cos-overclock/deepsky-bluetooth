...
/// device|epoch scope ごとに GATT operation を FIFO で 1 件ずつ直列化するキュー。
/// 同一 scope で実行中の operation がある間は後続を pending に積み、complete で次を開始する。
/// timeout 経過時は onTimeout を呼び、旧接続を破棄させる（古い接続を継続利用しない）。
