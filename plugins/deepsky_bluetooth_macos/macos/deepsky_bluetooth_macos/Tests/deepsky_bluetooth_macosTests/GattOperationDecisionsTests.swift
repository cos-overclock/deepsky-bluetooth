...
final class GattOperationDecisionsTests: XCTestCase {
  ...
  // MARK: - initializeDecision

  func testInitializeProceedsWhenForeground() {
    XCTAssertEqual(
      GattOperationDecisions.initializeDecision(isBackground: false), .proceed)
  }

  func testInitializeBackgroundNotSupported() {
    XCTAssertEqual(
      GattOperationDecisions.initializeDecision(isBackground: true), .backgroundNotSupported)
  }
}
